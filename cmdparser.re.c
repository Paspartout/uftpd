...
#define CMD_NOPARAM(KEYWORD) cmd.keyword = KEYWORD; \
	goto done;

#define CMD_STRING(KEYWORD) cmd.keyword = KEYWORD; \
	size_t len = p2-p1 > MAX_STRSIZE ? MAX_STRSIZE : p2-p1; \
	memcpy(&cmd.parameter.string, p1, len); \
	cmd.parameter.string[len] = '\0'; \
	goto done;

#define CMD_OPT_STRING(KEYWORD) cmd.keyword = KEYWORD; \
	size_t len = p2-p1 > MAX_STRSIZE ? MAX_STRSIZE : p2-p1; \
	if (len > 0) { \
	memcpy(&cmd.parameter.string, p1, len); \
	cmd.parameter.string[len] = '\0'; \
...
int main(int argc, char const* argv[])
{
	for (int i=1; i < argc; i++) {
		...
		printf("numbers: %d,%d,%d-%d,%d\n",
				...
				parsed.parameter.numbers[1],
				parsed.parameter.numbers[2],
				parsed.parameter.numbers[3],
				parsed.parameter.numbers[4]);
